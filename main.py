...
def main():
    ...
    while (
        ...
        print(
            "Use WASD keys to move the tiles (W: up, A: left, S: down, D: right)"
        )  # prompts the user to enter a move using the WASD keys,
        move = input(
            "Enter your move: "
        ).upper()  # converts the input to uppercase, and stores it in the variable move.
        ...
        try:  # Depending on the user's input, the appropriate movement function is called, modifying the grid accordingly.
            if move == "W":
                ...
                raise ValueError(
                    "Invalid move! Use WASD keys."
                )  # a ValueError is raised, and an error message is printed.
        ...
        # If a ValueError exception is raised within the try block, the code inside the except block is executed.
            ...
            print(
                e
            )  # the error message (e) is printed to inform the user that their input was invalid.
        ...
        if check_win(
            grid
        ):  # After each move, the check_win(grid) function is called to check if the player has won.
