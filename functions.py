...
def initialize_grid():
    ...
    for _ in range(2): # The underscore _ is a convention often used in Python to indicate a variable that is not going to be used within the loop.
        ...
        while grid[row][col] != 0: # This loop continues generating random row and column indices until it finds a position in the grid where the value is 0.
...
def print_grid(grid):
    for row in grid:
        print("|", end="") # | at beginning of each row. The end="" argument ensures that the subsequent prints in the same line instead of moving to the next line.
        for num in row:
...
def generate_new_cell(grid):
    ...
    if empty_cells: # empty_cells will contain the coordinates of all the empty cells in the grid, checks if empty_cells is not empty
        row, col = random.choice(empty_cells) # selects a random empty cell from empty_cells, assigns the row index to row and the column index to col.
        grid[row][col] = 2 # sets the value of the randomly chosen empty cell to 2, which generating a new cell with value 2 in a random empty position of the grid.
...
def move_left(grid):
    for i in range(4): # iterates over each row of the grid.
        for j in range(3): # iterates over each column of the grid from left to right, except for the last column.
            ...
            for k in range(j + 1, 4): # The variable k represents the index of the column being compared to the current column j which is the right column
...
def move_down(grid):
    for j in range(4): # iterates over each column of the grid.
        for i in range(3, 0, -1): # iterates over each row of the grid from buttom to top except for the first row.
            ...
            for k in range(i - 1, -1, -1): # range(i - 1, -1, -1) generates a sequence starting from i - 1 and ending at -1, iterating in reverse order.
